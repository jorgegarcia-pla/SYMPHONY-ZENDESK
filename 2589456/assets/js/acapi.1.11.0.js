...
class AudioCodesUA {
    constructor() {
        ...
        this.modes = { // default values for modes and fixes.
            ...
            ice_timeout_fix: 0,                // ICE gathering timeout (ms)
        ...
        this.userAgent = 'AudioCodes-WebRTC';
        this.wsSocket = null;
        ...
        AudioCodesUA.instance.getWR().connection.addEventListener(call.js_session.connection, 'track', (e) => {
            ...
            if (e.track.kind === 'video') {
                if (!call.hasEnabledReceiveVideo()) {
                    // Video call - audio answer, no sound in answer side issue. Patch for Chrome and Safari.
                    if (call.data['_video_call_audio_answer'] && ['chrome', 'safari'].includes(AudioCodesUA.instance.browser)) {
                        e.track.onmute = () => {
    ...
    _callOptions(sendVideo, isOutgoing, extraHeaders = null) {
        let options = {
            mediaConstraints: { 'audio': this.constraints[this.browser].audio },
            pcConfig: { iceServers: this.serverConfig.iceServers }
        };
        if (sendVideo) {
     ...
     */
    call(videoOption, call_to, extraHeaders = null) {
        ...
        let options = this._callOptions(videoOption === AudioCodesUA.instance.VIDEO, true, extraHeaders);
...
class AudioCodesSession {
    ...
    sendDTMF(tone) {
        ...
        if (AudioCodesUA.instance.getBrowser() === 'safari')
            useWebRTC = false;
     ...
     */
    answer(videoOption, extraHeaders = null) {
        // Convert boolean value to Symbol
        if (videoOption === false)
            videoOption = AudioCodesUA.instance.AUDIO;
        else if (videoOption === true)
        ...
        if (this.hasVideo()) {
            if (videoOption === AudioCodesUA.instance.AUDIO) {
                ...
                if (AudioCodesUA.instance.modes.video_call_audio_answer_fix && ['chrome', 'safari', 'firefox'].includes(AudioCodesUA.instance.browser)) {
        ...
        let options = AudioCodesUA.instance._callOptions(videoOption === AudioCodesUA.instance.VIDEO, false, extraHeaders);
     ...
     */
    async startSendingVideo(enabledReceiveVideo = true) {
    ...
    _renegotiate() {
        return new Promise((resolve, reject) => {
            ...
            if (!this.js_session.renegotiate({}, () => resolve(true))) {
                resolve(false);
            }
        });
    }

    async renegotiate(repeat = 30, delay = 500) {
        let i = 0;
        while (true) {
            AudioCodesUA.ac_log('AC: Renegotiate' + (i === 0 ? '' : ` try ${i + 1}`));
            if (await this._renegotiate())
                return;
            i += 1;
            if (i >= repeat) {
    ...
    async sendReInvite(showStreams = true) {
    ...
    // Under developing. It is not tested yet!
    async startScreenSharing() {
        AudioCodesUA.ac_log('AC: startScreenSharing');
        let displayStream = await AudioCodesUA.instance.getWR().getDisplayMedia();
        ...
        try {
            ...
            AudioCodesUA.ac_log('AC: startScreenSharing()', e);
            throw e;
        }
        this._setEnabledSendVideo(true);
        if (videoAdded || !this.hasSendVideo()) {
            await this.renegotiate();
        } else {
            AudioCodesUA.ac_log('AC: No need renegotiation');
        }
    }
    ...
    async _onEndedScreenSharing() {
        ...
        let localStream = this.getRTCLocalStream();
        if (screenSharing.sendVideo) {
            AudioCodesUA.ac_log('AC: screen sharing stopped - restore previosly sending video track');
            ...
            await this.stopSendingVideo();
        }
        if (AudioCodesUA.instance.listeners.callEndedScreenSharing) {
            AudioCodesUA.instance.listeners.callEndedScreenSharing(this);
        }
    }
